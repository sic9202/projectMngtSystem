...
function getShareDatasetList(chk, page){
	var tag = "";
	...
	$.postDataGetJSON("/getShareDatasetList.do", {
		...
		if(r.list.length > 0){
			...
			for(var i = 0; i < r.list.length; i++){
				...
				tag += 	'<div class="data-view-result" style="cursor: pointer;" onclick="shareDatasetView('+dataset_idx+')">'
...
function shareDatasetView(dataset_idx){
	$("#moveForm").attr("action", "/shareDatasetView.do");
	...
	$.postDataGetJSON("/getDatasetLayout.do", {"dataset_idx":dataset_idx}, function(r){
		...
		if(r.dataset_layout_info.length > 0){
			for(var i = 0; i < r.dataset_layout_info.length; i++){
				...
				var column_desc = "-";
				if(r.dataset_layout_info[i].column_desc != null){
					column_desc = r.dataset_layout_info[i].column_desc;
				}
...
function addMyDataset(dataset_idx){
	$.postDataGetJSON("/addMyDataset.do", {"dataset_idx":dataset_idx}, function(r){
		if(r.status == 1){
			if(r.isExist == 1){
				alert("이미 관심목록에 등록되어있거나, 보유한 데이터셋입니다.");
			...
			alert("관심목록 등록에 실패했습니다.");
			shareDatasetView(dataset_idx);
		}
	});
}
