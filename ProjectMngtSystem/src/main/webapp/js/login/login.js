...
function check_user() {
	if(value_check()){
		...
		$.postDataGetJSON("/check_user.do", {"id":id, "user_pwd":user_pwd}, function(r){
			if(r.use_yn == 1) {
				alert("탈퇴한 회원입니다.");
				return false;
			} else {
				if(r.status == 1){
					if(r.list[0].user_type == '2'){
						window.location.href = encodeURI("/userMngtList.do");
					}else{
						if(r.list[0].pass_check == 1){
							window.location.href = encodeURI("/chage_pw.do");
						}else{
